...
$().ready(function(){
	...
	$("#myOrder").on("click",function (){
		window.location.href = "${ctx}/webApp/toMyMerchantOrder";
	});
	
	$("#myCheckCode").on("click",function (){
		window.location.href = "${ctx}/webApp/toMyMerchantCheckCode";
	});
	//我的统计
	$("#myStatistic").on("click",function (){
		window.location.href = "${ctx}/webApp/toMyMerchantStatistic";
	});
	//退出
	$("#loginOut").on("click",function (){
		if(confirm("是否要退出？")){
			showLoading("退出中...");
			$.post("${ctx}/webApp/merchantLoginOut",function(data){
				$.cookie("isMerchantLogin",false, { path: '${ctx}' });
				$.cookie("target_merchant_herf",null, { path: '${ctx}' });
...
<%-- 	<img src="${ctx}/static/front/images/3.png" width="80" height="80"/> --%>
	<div class="mybg">&nbsp;&nbsp;欢迎 ${bussiness.name} 的使用</div>
	...
	<a href="javascript:;" class="list-group-item" id="myCheckCode" style="font-size: 17px;line-height: 25px;">
		<span style="color:#2885EC;"><span class="glyphicon glyphicon-check"></span></span>&nbsp;&nbsp;&nbsp;&nbsp;消费验码
	</a>
	<br/>
