...
<body>
	...
	<script type="text/javascript">
		...
		function orderDetail(orderId){
			window.location.href = "${ctx}/webApp/user/toMyOrderDetail/"+orderId;
		}
		...
		function init(pageNum) {
			...
			//循环数据
			$.post("${ctx}/webApp/user/myOrder",{currentPage:pageNum},function(data) {
				if(data == "-100"){
...
// 									listContent +='<div class="panel-body" style="background-color:#eee;"></div>';
									...
									listContent +='</tr>';
									listContent +='<tr>';
									listContent +='<td>预约方式:</td>';
									listContent +='<td>';
									if(order.orderOwn == 1){
										listContent +='<span>上门</span>&nbsp;';
									}else if(order.orderOwn == 2){
										listContent +='<span >到店</span>&nbsp;';
									}
									listContent +='</td>';
									listContent +='</tr>';
									listContent +='<tr>';
									if(order.isRecommend == 0 && order.status == 7){
										listContent +='<td><a href="javascript:;" onclick="orderDetail('+order.id+')"><span class="glyphicon glyphicon-duplicate"></span>&nbsp;订单详情</a></td>';
										listContent +='<td><a href="javascript:;" onclick="goRecommend('+order.id+')"><span class="glyphicon glyphicon-duplicate"></span>&nbsp;去评价</a></td>';
									}else if(order.isRecommend == 1 && order.status == 7){
										listContent +='<td><a href="javascript:;" onclick="orderDetail('+order.id+')"><span class="glyphicon glyphicon-duplicate"></span>&nbsp;订单详情</a></td>';
										listContent +='<td><a href="javascript:;" onclick="goRecommend('+order.id+')"><span class="glyphicon glyphicon-duplicate"></span>&nbsp;已评价</a></td>';
									}else{
